...
api_keys = {}
api_key = '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'  #直接在這裡改
...
def handle_text_message(event):
  ...
  with open(f"sturesp/okQ/{user_id}.json", mode="r",
            encoding="utf8") as okQ_dic:
    okQ_dic = json.load(okQ_dic)
  # 定義 okQ 答對的題庫
...
👉向機器人問問題""")
  ...
  # 讀取json題庫檔案
  with open(f"sturesp/allresp/{user_id}.json", 'r') as f:
    allresp = f.read()
...
def sturecord():
  with open(os.path.join('gptLBproj/gptLBproj/templates/sturecord.html'),
            'r',
...
if os.path.isfile('gptLBproj/gptLBproj/templates/sturecord.html'):
...
# 創建一個dictionary來儲存所有使用者的DataFrame
user_tables = {}

# 逐一讀取每個txt檔案，整理成DataFrame，並存儲在user_tables中
...
# 將每個使用者的DataFrame轉換成HTML表格，並連接起來
html_tables = []
...
# 在sturecord.html檔案的末尾繼續添加HTML表格
with open('sturecord.html', 'a+', encoding='utf-8') as f:
  # 新增下拉選單
  options = ''
  ...
  html = f"{select}<div id='table-container' style='text-align:center; padding-left: 50px;'>{all_html_tables}</div>"

  # 在<head>標籤中添加JavaScript程式碼，當下拉
