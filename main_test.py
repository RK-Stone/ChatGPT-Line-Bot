...
with open("Questions.json", encoding='utf8') as Q:
  Questions = Q.read()
  questions_dic = json.loads(Questions)
...
def handle_text_message(event):
  ...
  with open("all_resp"+user_id+".json", mode="a+", encoding='utf8') as all_resp:
    all_resp.write(f'{user_id}: {text}')

  with open("okQ_resp"+user_id+".json", mode="a+", encoding='utf8') as okQs_resp:
    okQ_resp = okQs_resp.read()   #答對的題庫 若還沒有就可在此先創建
  ...
  if text.startswith('「題目」'):
    if len(okQ_resp) == len(questions_dic):  # 若所有題目都回答正確
      msg = TextSendMessage(text="你已經完成所有題目囉！")
    else:
      ran_numQ = random.choice(numsQ)
      ran_q = questions_dic["q" + str(ran_numQ)]
...
#調用答案
    ...
    if 'A' == ran_q['a']:
      ...
      with open("okQ_resp"+user_id+".json", mode="a+", encoding='utf8') as okQ_resp:
        okQ_resp.write("q")
    else:
...
def index():
  with open(os.path.join('gptLBproj/gptLBproj/templates/index.html'), 'r',
...
def stuall():
  with open(os.path.join('gptLBproj/gptLBproj/templates/stuall.html'), 'r',
...
def stuone():
  with open(os.path.join('gptLBproj/gptLBproj/templates/stuone.html'), 'r',
